...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf, norm_type, act_type='selu', use_dropout=False, n_blocks=6, padding_type='reflect', gpu_ids=[]):
        ...
        self.convfinal1 = nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        self.convfinal2 = nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        self.convfinal3 = nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        self.convfinal4 = nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        self.convfinal5= nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        self.convfinal6= nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        self.convfinal7= nn.Conv2d(self.ngf, 3, kernel_size=7, padding=0).cuda(self.gpulist[0])
        
       
        self.t = nn.Tanh().cuda(self.gpulist[0])
    ...
    def forward(self, input):
        ...
        input1 = self.convfinal1(input)
        input1 = self.t(input1)
        input2 = self.convfinal2(input)
        input2 = self.t(input2)
        input3 = self.convfinal3(input)
        input3 = self.t(input3)
        input4 = self.convfinal4(input)
        input4 = self.t(input4)
        input5 = self.convfinal5(input)
        input5 = self.t(input5)
        input6 = self.convfinal6(input)
        input6 = self.t(input6)
        input7 = self.convfinal7(input)
        input7 = self.t(input7)
        return torch.concat((input1,input2,input3,input4,input5,input6,input7),1)
        # return input1

class ResnetGenerator_forgery(nn.Module):
    def __init__(self, input_nc, output_nc, ngf, norm_type, act_type='selu', use_dropout=False, n_blocks=6, padding_type='reflect', gpu_ids=[]):
        assert(n_blocks >= 0)
        super(ResnetGenerator_forgery, self).__init__()
